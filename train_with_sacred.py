...
def config():
    checkpoints = './ourdata/'
    data_folder = '/home/alex/data/ourdata/train/'
    ...
    optimizer = 'adam'
    resume = True
    weights ='/media/alex/4/models/final/1-0.1.tar'
    rescale_rot = 1.0
    rescale_transl = 2.0
    precision = "O0"
    norm = 'bn'
...
def train(model, optimizer, rgb_img, refl_img, target_transl, target_rot, loss_fn, point_clouds, loss,rgb,cam_calib):
    ...
    if loss == 'points_distance' or loss == 'combined':
        ...
        losses = loss_fn(target_transl, target_rot, transl_err, rot_err,cam_calib)
...
def main(_config, _run, seed):
    ...
    for k,v in model.named_parameters():
        ...
        if k in ['module.fc1_rot.bias','module.fc1_rot.weight','module.fc2_rot.bias','module.fc2_rot.weight']:
            v.requires_grad = True
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(ValImgLoader):
            ...
            poseloss+=loss['transl_loss'].item() * len(sample['rgb'])
            poseloss1+=loss['rot_loss'].item() * len(sample['rgb'])
            val_iter += 1

        print("------------------------------------")
        print('total val loss = %.3f' % (total_val_loss / len(dataset_val)))
        print(f'total traslation loss: {poseloss / len(dataset_val)} ')
        print(f'total rotation loss: {poseloss1 / len(dataset_val)} ')
        print(f'total distance loss: {2*(total_val_loss-poseloss-poseloss1*0.5) / len(dataset_val)} ')
